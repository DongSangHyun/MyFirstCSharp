...
namespace MyFirstCSharp
{
    ...
    // 클래스의 코드 작성 위치
    // . 클래스를 생성할 경우 프로젝트 에 cs 파일로 추가 하는 방법이 있고. 
    //   소스 내에서 클래스를 수동으로 생성 하는 방법이 있다. 
    //   여러 개발자가 개발을 할 경우 공통으로 사용하는 클래스 인 경우 
    //   솔루션 탐색기 에서 확인 할 수 있도록 파일 형태의 클래스를 생성 하는것이 좋고
    //   개발자 가 단독으로 사용할 클래스 인 경우 소스 내에서 생성하여 사용 할수 있다. 
    class NewClass
    { 
        // 새로운 클래스의 생성.
        ...
        {
            // 로직이 실행되는 메서드 
            Random randonm = new Random();  // 마이크로 소프트 에서 만들어서 제공하는 클래스.
            ...
            //CHP27.FindValue2(); // 다른 클래스에서 접근이 불가능한 메서드 private Method

            // FindValue() 를 실행하고 FindValue2() 를 순차적으로 실행해야 하는경우
            // FindValue2() 를 외부에서 접근하지 못하도록 Private 로 접근제한을 두고
            // FindValue() 는 외부에서 접근 가능하도록 Public 으로 접근 제한을 두면서 FindValue2() 를 호출 함으로서
            // 순차적인 로직을 실행 할 수 있도록 한다. 
            
            // 캡술화
            // 최소한 의 소스, 코딩 만 외부에 공개 함으로서 프로그램의 무결성과 안전성을 유지 하도록 하는 프로그래밍 방식 
