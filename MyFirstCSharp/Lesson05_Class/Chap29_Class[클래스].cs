...
namespace MyFirstCSharp.Lesson05_Class
{
    /*
       ...
       . 프로그램을 구현하는 설계되 역할 (설계도를 참조해서 실체를 만들어내는 과정)
       . 설계도를 보고 같은 모양의 단위 부품을 필요에 의해 여러번 만들 수 있듯이
         클래스 라는 파일을 생성해 두고 프로그램 내에서 필요에 따라 
         객체 (실체 ) 로 인스턴스 화 해서 사용할 수 있다.
       . 객체화(인스턴스화) : 클래스(설계도, 레시피) 를 바탕으로 실제로 구현 되어야할
                               내용을 실행 할 수 있도록 만드는 단계 
       . 외부에서 선언후 사용하는 것이 목적 기본 상태는 Public 
         Private 형태로는 만들수 없다. (중첩 클래스 일경우는 Private 제한자 설정이 가능)
    ...
    */
    ...
    internal class Chap29_Class
    {
        ...
        // 객체 생성 시 가장 먼저 메모리 에 등록 (Heap, Data) 
        public string sValue;
        public Random random = new Random(); // Radom 클래스를 Cahp29 _Class 의 필드 멤버로 사용하기 위해 등록
        ...
        public Chap29_Class()
        {
            // 클래스 이름과 같은 일종의 메서드 
            // 1. 반환 유형을 설정 할수 없다. 
            // 2. 해당클래스(this) 가 객체화 (new )Cahp29_Class() 될때 호출 되는 코드의 모음.
            // 3. 주로 클래스의 멤버를 초기화 하는 용도로 많이 사용됨. (초기화 = 비어있는객체의 이름에 값을 할당)
        ...
        public Chap29_Class(string sValue)
        {
            ...
            // 필요에 따라 유연하게 클래스 멤버를 초기화 하는 기능으 구현 할 수 있다.
        ...
        // 필드 멤버     : 클래스에서 사용되는 변수, 객체 를 관리하는 멤버
    ...
    class NewClass
    {
        public NewClass()
        {
            ...
            // CHAP29 라는 객체 이름을 Stack 에 등록 하여 
            // Heap 에 있는 주소 를 CHAP29 객체 (Stack) 에 전달.
            Chap29_Class CAHP29 = new Chap29_Class(); // 기본 생성자를 호출. 
            MessageBox.Show(CAHP29.sValue); // 안녕하세요 필드 멤버 인스턴스 문자열 변수입니다."
            Chap29_Class CHAP29_T = new Chap29_Class("셋팅할 문자열 할당");
            MessageBox.Show(CHAP29_T.sValue); // "셋팅할 문자열 할당"
