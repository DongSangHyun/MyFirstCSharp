...
{
    // 예외 처리 Exception 
    // 프로그램 구동 시 발생 할 수 있는 시스템 오류 를 검출 하고. 
    ...
    public partial class Chap23_TryCatchFinally : Form
    {
        ...
        private void btnSystemError_Click(object sender, EventArgs e)
        {
            ...
            // i 가 3이 되는 순간. index 를 벗어 나므로 시스템 오류가 발생한다. 
            for (int i = 0; i < 5; i++)
            {
        ...
        {
            // 예외 처리 
            // * try 와 catch 구문은 반드시 쌍으로 구현 되어야 한다. 
            ...
            try
            {
                ...
                {
                    MessageBox.Show(iValues[i].ToString());
                    // 오류가 발생하는 순간 로직이 catch 로 이동.
                }
            }
            catch
            {
                // 예외 상황이 발생 하였을 경우 대체 할수 있는 로직이 등록되는 부분.
                // 예외 상황을 알리는 메세지를 사용자 에게 제공 함으로서 
                // 시스템의 신뢰성 을 높이는 문법. 
        ...
        {
            // 메서드를 감싸고 있는 try Catch 의 경우. (메인 로직)
            // 메서드 내에서 오류 가 발생 시 
            // 메서드 애부에서 예외 상황을 발생 시켰으면 메인 로직 입장에서는 정상 완료. 
        ...
        bool SetErrorMethod()
        {
            try
            {
                ...
                return true; // 정상 적으로 처리 되었을 경우. 
            }
            catch
            {
                MessageBox.Show("서브 메서드에서 오류 가 발생하였습니다.");
                return false; // 메서드 기능 동작 중 오류 가 발생하였을 경우.
            }
        }
        ...
        {
            // Exception ? 
            // 발생할 수 있는 시스템 오류 의 원인을 확인하여 
            // 프로그램 개발자 또는 사용자 에게 보고 할 수 있도록 전달하는 클래스
            // 오류가 검출되는 상황에 맞는 Exception 클래스를 별도로 사용할 수 있으나. 
            // 통상적으로 Exception 클래스 (모든 예외상황을 검출하는 클래스) 를 사용하며
            // 자세한 오류 내역을 검출 하고자 할때 별도의 Exception 클래슴를 혼합하여 사용 가능.
            ...
            // 구현 하려고 하는 로직에서 오류 가 발생 할것 같은 Exception 클래스를 선택.\
            try
            {
                for(int i = 0; i < 5; i++)
                {
            ...
            //catch(UriFormatException IndexEx)
            //{
